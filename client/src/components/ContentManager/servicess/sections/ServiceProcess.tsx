...
const ServiceProcess = ({ index, register, control }: ServiceSectionProps) => {
  ...
  return (
    <section className="space-y-4 rounded-lg border border-slate-200 p-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-slate-700">Highlight Heading</label>
          <input
            type="text"
            className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
      ...
      <div className="space-y-2">
        <p className="text-sm font-medium text-slate-700">Steps ({fields.length})</p>
            ...
            <div className="grid gap-4 border-t border-slate-200 p-4 md:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  accept="image/*"
                  className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
                  {...register(`services.${index}.ServiceProcess.stepCard.${stepIndex}.imgSrc`)}
                />
              </div>
              <div>
                ...
                <input
                  type="text"
                  className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
                  {...register(`services.${index}.ServiceProcess.stepCard.${stepIndex}.title`)}
                />
              </div>
              <div className="md:col-span-2">
                ...
                <input
                  type="text"
                  className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
                  {...register(`services.${index}.ServiceProcess.stepCard.${stepIndex}.description`)}
                />
              </div>
            </div>
          </details>
        ...
        <div className="flex justify-end">
          <button
            type="button"
            onClick={() => append({ imgSrc: undefined as unknown as FileList, title: "", description: "" })}
